...
load_results <- function(seed) {
  ...
  acc_res[1,] = unlist(simdata_kalman[2:4,1], use.names=FALSE)
  ...
  acc_res[2,] = unlist(simdata_pmmh[2:4,1], use.names=FALSE)
  ...
  acc_res[3,] = unlist(simdata_pmmh[2:4,1], use.names=FALSE)
  ...
  simdata_pmmh <- read_csv(paste("data/SDEMEM OU/cpmmh/sim_data_", job, ".csv", sep = ""))
  acc_res[4,] = unlist(simdata_pmmh[2:4,1], use.names=FALSE)
  ...
  simdata_pmmh <- read_csv(paste("data/SDEMEM OU/cpmmh/sim_data_", job, ".csv", sep = ""))
  acc_res[5,] = unlist(simdata_pmmh[2:4,1], use.names=FALSE)
...
print(round(apply(res,c(2,3),sd),2))
