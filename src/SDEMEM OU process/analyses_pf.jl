...
chain_1 = Matrix(CSV.read("data/SDEMEM OU/pf/chain_1_"*job*".csv"; allowmissing=:auto))'
chain_2 = Matrix(CSV.read("data/SDEMEM OU/pf/chain_2_"*job*".csv"; allowmissing=:auto))'
chain_3 = Matrix(CSV.read("data/SDEMEM OU/pf/chain_3_"*job*".csv"; allowmissing=:auto))'

loglik2_loglik_y = Matrix(CSV.read("data/SDEMEM OU/pf/loglik2_loglik_y_"*job*".csv"; allowmissing=:auto))
loglik2_loglik_ϕ = Matrix(CSV.read("data/SDEMEM OU/pf/loglik2_loglik_ϕ_"*job*".csv"; allowmissing=:auto))
...
accept_vec = Matrix(CSV.read("data/SDEMEM OU/pf/accept_vec_"*job*".csv"; allowmissing=:auto))'
...
# acceptance rate
sum(accept_vec[1,burn_in:end]/R*100)

sum(accept_vec[2,burn_in:end]/(M_mixtures*(R-burn_in))*100)

sum(accept_vec[3,burn_in:end]/R*100)
