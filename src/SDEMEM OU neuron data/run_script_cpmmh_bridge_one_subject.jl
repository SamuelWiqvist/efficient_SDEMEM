...
# load and set start values and covaraince matrix
...
#startval_log_σ_ϵ = -23.097444012292712
#startval_ϕ = [-2.87197 -0.640331 -0.738921]


ll = @time bridge_pf(y, exp(startval_log_σ_ϵ), startval_ϕ, dt, u_prop_old, u_resample_old, nbr_particles, true)
sum(ll)
...
α_star_ϕ = 0.25
α_star_σ_ϵ = 0.25
log_λ_i_ϕ = log.(1*ones(M_subjects))
log_λ_i_σ_ϵ = log(1)
update_interval = 1
α_power = 0.7
start_update = 100
...
job = string(M_subjects)*"_"*string(nbr_particles[1])*"_"*string(ρ)
